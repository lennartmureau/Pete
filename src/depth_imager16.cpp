...
void callback(const sensor_msgs::PointCloud2ConstPtr& msg) {
  ...
  for (uint16_t pixelX = 0; pixelX < IMAGE_WIDTH; pixelX++) {
    for (uint16_t pixelY = 0; pixelY < IMAGE_HEIGHT; pixelY++) {
      ...
      outputImage[(pixelX + pixelY * IMAGE_WIDTH) * 2] = top / bottom;
      outputImage[(pixelX + pixelY * IMAGE_WIDTH) * 2 + 1] = int(top / bottom) >> 8;
    }
  }
  ...
  */
  ...
  outMsg.step = (IMAGE_WIDTH * 2);
  std::vector<uint8_t> output(outputImage, outputImage + IMAGE_WIDTH * IMAGE_HEIGHT * 2);
  outMsg.data = output;
  pub.publish(outMsg);
